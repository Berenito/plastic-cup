...
from utils.swiss_system import generate_group_round
...
# Generate Round 1
game_ids_1 = [f"G-1-{i}" for i in range(n_games_per_round)]
games_0 = pd.DataFrame(columns=["Round", "Team_1", "Team_2", "Score_1", "Score_2"])
ratings_0 = pd.Series(0, index=team_names)
games_1 = generate_group_round(ratings_0, games_0, game_ids_1, 2)
...
# Generate Round 2
game_ids_2 = [f"G-2-{i}" for i in range(n_games_per_round)]
games_2 = generate_group_round(ratings_1, games_1, game_ids_2, 2)
